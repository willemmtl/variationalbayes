...
function KLOptim(F::iGMRF, Y::Vector{Vector{Float64}})
    ...
    function KLDivergence(θ::Vector{<:Real})
        ...
        a = θ[m+4];
        b = θ[m+5];
        logDensityApprox(x::DenseVector) = logpdf(MvNormal(η, Σ), x) + logpdf(Gamma(a, b), x[1])
    ...
    θ₀ = [η₀..., Σ₀[1], mean(Σ₀[2:end]), 1, 100];
    res = optimize(KLDivergence, θ₀, Optim.Options(iterations=10000));
